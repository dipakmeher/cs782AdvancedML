...
def process_chunks(chunk_files, args, prompt_template, resolved_entities, aux_descriptions, verification=False):
    for fname in chunk_files:
        ...
        while attempt < max_retries and not success:
            ...
            fallback_1 = ""
            fallback_2 = """
            Your previous output was invalid JSON because you added explanations after quoted items.
            Do not add commentary. List items must be pure quoted strings only.
            """
            fallback_3 = """
            Fix the JSON error: list items must be ONLY quoted strings. No explanations or sentences.
            If unsure, return empty lists. Output JSON only.
            """
            fallback_4 = """
            STRICT MODE: valid JSON only. No commentary, no reasoning, no text outside JSON.
            List items must be single quoted strings or empty.
            """
                
            fallback_variants = [
                    fallback_1 + full_prompt,
                    fallback_2 + "\n\n" + full_prompt,
                    fallback_3 + "\n\n" + full_prompt,
                    fallback_4 + "\n\n" + full_prompt
                    ]
            ...
            try:
                #log(f"Prompt length (chars): {len(full_prompt)}", args.log_file)
                prompt_to_use = fallback_variants[min(attempt, len(fallback_variants)-1)]
                log(f"Prompt length (chars): {len(prompt_to_use)}", args.log_file)
                
                start_time = time.time()
                #result_raw = run_ollama_inference(full_prompt, args.model, args.ollama_url, timeout=120)
                result_raw = run_ollama_inference(prompt_to_use, args.model, args.ollama_url, timeout=300)
